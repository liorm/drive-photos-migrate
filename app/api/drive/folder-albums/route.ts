...
/**
 * POST /api/drive/folder-albums - Get folder-to-album mappings with lazy discovery
 *
 * Request Body:
 * - folderIds: Array of folder IDs
 * - folderNames: Array of folder names (same order as IDs, for lazy discovery)
 */
async function handlePOST(request: NextRequest) {
  ...
  // Get request body
  let body: { folderIds?: string[]; folderNames?: string[] };
  try {
  ...
  logger.info('Get folder-album mappings request (POST)', {
    requestId,
    userEmail,
    folderCount: folderIds.length,
    hasNames: folderNames.length > 0,
  ...
  logger.info('Folder-album mappings retrieved successfully (POST)', {
    requestId,
    userEmail,
    totalRequested: folderIds.length,
    mappingsFound: Object.keys(mappingsResult).length,
